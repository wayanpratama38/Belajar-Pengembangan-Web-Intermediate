...
export default class LightboxManager {
    ...
    openLightbox(story) {
      ...
      this.originalCard = document.querySelector(`.story-card[data-story-id="${story.id}"]`);
    ...
    updateLightboxContent(story) {
      ...
      const descriptionEl = this.lightboxContent.querySelector('.lightbox-description');
      ...
      if (dateEl) dateEl.textContent = new Date(story.createdAt).toLocaleDateString();
    }
    ...
    closeLightboxWithViewTransition() {
      if (!this.isOpen || !this.lightbox || !this.lightboxContainer || !document.startViewTransition) {
        this.closeLightbox();
        return;
      }
